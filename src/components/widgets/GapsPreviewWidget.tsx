        ...
        <div>
          ...
          <div className="space-y-2.5 pt-3">
            {topGaps.map(gap => (
              <div
                key={gap.id}
                className="p-3 rounded-xl bg-surface-2/40 border border-white/[0.06] hover:border-white/15 hover:bg-surface-2/70 transition-all space-y-1.5 group cursor-pointer"
              >
                <div className="flex items-start justify-between gap-2.5">
                  <span className="text-xs font-semibold text-text leading-snug group-hover:text-white transition-colors">
                    {gap.title}
                  </span>
                  <span
                ...
                <div className="flex items-center gap-2 text-[11px] text-text-muted">
                  <span className="font-mono text-slate-300 font-medium">{gap.impact}</span>
                  <span className="text-white/20">·</span>
                  <span className="px-1.5 py-0.2 rounded bg-surface border border-white/10 text-[10px] capitalize font-medium">
                    {gap.effort}
                  </span>
                  <span className="text-white/20">·</span>
                  <span className="text-cyan-400 text-[10px] font-medium">{gap.category}</span>
                </div>
              </div>
            ))}
          </div>
